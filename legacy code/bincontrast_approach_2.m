...
for pen = 1:length(list)
    ...
    for e = 1:length(STIM.depths)
        ...
        cbins = [0, 0; 0.10 0.15; 0.20, 0.45; 0.50, 1]; % bins of contrast levels
        numC = length(cbins);
        ...
        if normalize == 1 % if normalizing, just create a temporary UNIT struct
            try
...
clearvars -except sdfWin Trls PEN IDX UNIT flag_save N uct dataset cbins
...
var7c = @(oldvar) mat2cell(oldvar(:), [fix(numel(oldvar)/7) *[1, 1, 1, 1,1,1], numel(oldvar)-6*fix(numel(oldvar)/7)], 1);     % Create New Matrix From Original Vector
occGroups = var7c(I2); % rows are: low med high

occLengths = [numel(occGroups{1,1}),numel(occGroups{2,1}),numel(occGroups{3,1}),numel(occGroups{4,1}),numel(occGroups{5,1})...
    numel(occGroups{6,1}),numel(occGroups{7,1})];
...
save(sprintf('Binned_%s_norm',dataset),'IDX','UNIT','PEN','Trls','N','uct','sdfWin','dataset');
    try 
    cd('D:\')
    save(sprintf('Binned_%s_norm',dataset),'IDX','UNIT','PEN','Trls','N','uct','sdfWin','dataset');
    catch
