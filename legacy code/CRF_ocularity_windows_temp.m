...
% strings
curves = {'de','nde','bin'};
labels = {'0','15-30','40-60','80-100'}; % cbins
...
global Data
Data(1,:) = x;
figure('position',[148,607,1501,238]);
for o = 1:length(occGroups) % for each group of ocularity
    ...
    %s2 = semilogx(predictions,prd(2,:),'color',[.4 .4 .4],'linestyle','-','linewidth',1.75,'HandleVisibility','off');  hold on
    s3 = semilogx(predictions,prd(3,:),'color','b','linestyle','-','linewidth',2,'HandleVisibility','off');  
    
    s4 = semilogx(Data(1,:),DE,'o','color','k','linewidth',2,'markersize',5);
    if error == true
        ci1 = errorbar(Data(1,:),de_all(o).avg(:,tw),de_all(o).err(:,tw),'k');
    ...
    if error == true
        ci2 = errorbar(Data(1,:),bin_all(o).avg(:,tw),bin_all(o).err(:,tw),'b');
    ...
    if o == 1
        ylabel('impulses per sec');
        xlabel('contrast bins');
        xticklabels({'1','2','3','4'});
    else
    ...
    if length(occGroups) == 3
        if o == 1
            ...
            xlabel('Stimulus contrast'); ylabel(sprintf('Impulses per sec'));
            xticklabels(labels);
        elseif o == 2
            occRange = abs([occValues(occGroups{o,1}(1)),occValues(occGroups{o,1}(end))]);
