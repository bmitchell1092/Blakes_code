...
%% Panel 1: figure 2
figure('position',[294.3333333333333,39.666666666666664,640,599.3333333333333]);
refwin = -50:600;
clear c L
for L = 1:3
    subplot(3,1,L)
    ...
    ax = gca;
    ax.FontSize = 12; 
    set(gca,'ylim',[-10 ylimit],'Box','off','TickDir','out','linewidth',1.5)
...
for L = 1:3
subplot(3,1,L)
bar(sAVG.BIN.aMUA.pc.coll_layers.data(:,1,L),.8,'FaceColor',[0.8500, 0.3250, 0.0980],'EdgeColor','k','LineWidth',0.8);
hold on
errorbar(sAVG.BIN.aMUA.pc.coll_layers.data(:,1,L),sAVG.BIN.aMUA.pc.coll_layers.error(:,1,L),'o','marker','none','color','k');
bar(sAVG.DE.aMUA.pc.coll_layers.data(:,1,L),0.4,'FaceColor',[0, 0.4470, 0.7410],'EdgeColor','k','LineWidth',0.8);
errorbar(sAVG.DE.aMUA.pc.coll_layers.data(:,1,L),sAVG.DE.aMUA.pc.coll_layers.error(:,1,L),'o','marker','none','color','k');
set(gca,'box','off','linewidth',1.5);
    ax = gca;
    ax.FontSize = 12; 
xticklabels([]);
...
cd('C:\Users\bmitc\OneDrive\4. Vanderbilt\Maier Lab\Figures\VSS\')
saveas(gcf, strcat('panel_1_fig3_v3', '.svg'));
