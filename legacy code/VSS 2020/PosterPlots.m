...
%% Panel 1: figure 2
figure('position',[360,107.6,201,510]);
refwin = -50:600;
clear c L
for L = 1:3
    subplot(3,1,L)
...
cd('C:\Users\bmitc\OneDrive\4. Vanderbilt\Maier Lab\Figures\VSS\')
saveas(gcf, strcat('panel_1_fig2', '.svg'));
...
labels = {22, 45, 90}; format bank;

clear L c
for L = 1:3
subplot(3,1,L)
bar(sAVG.BIN.aMUA.pc.coll_layers.data(2:4,2,L),.8,'FaceColor',[0.8500, 0.3250, 0.0980],'EdgeColor','k','LineWidth',0.8);
hold on
errorbar(sAVG.BIN.aMUA.pc.coll_layers.data(2:4,2,L),sAVG.BIN.aMUA.pc.coll_layers.error(2:4,2,L),'o','marker','none','color','k');
bar(sAVG.DE.aMUA.pc.coll_layers.data(2:4,2,L),0.4,'FaceColor',[0, 0.4470, 0.7410],'EdgeColor','k','LineWidth',0.8);
errorbar(sAVG.DE.aMUA.pc.coll_layers.data(2:4,2,L),sAVG.DE.aMUA.pc.coll_layers.error(2:4,2,L),'o','marker','none','color','k');
set(gca,'box','off','linewidth',1);
xticklabels([]);
ylim([0 60]);
%xlabel('% contrast')
...
cd('C:\Users\bmitc\OneDrive\4. Vanderbilt\Maier Lab\Figures\VSS\')
saveas(gcf, strcat('panel_1_fig3', '.svg'));
...
cd('C:\Users\bmitc\OneDrive\4. Vanderbilt\Maier Lab\Figures\VSS\')
saveas(gcf, strcat('panel_2_fig1', '.svg'));

%% Panel 2: figure 2

figure('position',[402,41,493,599]);
cIndex = 4;
...
%vline(tmp.STIM.off,'k','offset')
   if L == 1
    ...
    elseif L == 2
            ...
            title('Infragranular','FontSize',12);
            lgd = legend('LSM','QSM','location','northeast');
            lgd.FontSize = 8;
            xlabel('time (ms)','FontSize',12);
            ylabel(sprintf('difference from \nbinocular response'),'FontSize',12);
    end
end
...
cd('C:\Users\bmitc\OneDrive\4. Vanderbilt\Maier Lab\Figures\VSS\')
saveas(gcf, strcat('panel_2_fig2', '.svg'));


%% Panel 2: figure 3
labels = {0, 22, 45, 90,[],0,22,45,90}; format bank;
figure('Position', [83,179,924,246]);
...
cd('C:\Users\bmitc\OneDrive\4. Vanderbilt\Maier Lab\Figures\VSS\')
saveas(gcf, strcat('Panel_2_fig3', '.svg'));
...
plot(squeeze(sAVG.DI.aMUA.pc_QSM.coll.aligned(di,2,:)),corticaldepth,'linewidth',1.5,'linestyle',':','color','k');
grid on
xlim([-5 80]);
%hline(0,'-.','BOL4')
ylim([-7 17])
...
hold on
plot(squeeze(sAVG.DI.aMUA.pc_QSM.coll.aligned(di,2,:)-(sAVG.DI.aMUA.pc.coll.aligned(di,2,:))),corticaldepth,'linewidth',1.5,'linestyle',':','color','k');
grid on
xlim([-10 30]);
%hline(0,'-.k')
ylim([-7 17])
...
cd('C:\Users\bmitc\OneDrive\4. Vanderbilt\Maier Lab\Figures\VSS\')
saveas(gcf, strcat('panel_3_fig1', '.svg'));
...
hline(0,'k')
   if L == 1
    ...
    elseif L == 2
            ...
            lgd.FontSize = 8;
            xlabel('time (ms)','FontSize',12);
            ylabel(sprintf('difference from \ndichoptic response'),'FontSize',12);
    end
end
...
cd('C:\Users\bmitc\OneDrive\4. Vanderbilt\Maier Lab\Figures\VSS\')
saveas(gcf, strcat('panel_3_fig2', '.svg'));
...
hold off
    if L == 1
        ...
        xticklabels(labels)
        xtickangle(45)
        xlabel('stimulus contrast (DE | NDE)','FontSize',12)
    elseif L == 2
            title('Granular','FontSize',12);
            xticklabels([])
            yticklabels([])
...
cd('C:\Users\bmitc\OneDrive\4. Vanderbilt\Maier Lab\Figures\VSS\')
saveas(gcf, strcat('panel_3_fig3', '.svg'));