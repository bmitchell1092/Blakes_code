            ...
            for c = 1:length(stimcontrast) % for each contrast level
                switch moncond{mon}
                    case 'DE_PS'
                        ...
                        n = binTrials(c,1); % number of trials to keep
                        f = find(trls); % find the location of the 1's
...
%         end
        ...
        if normalize == 1
            mn       = min(monSDF(:,1,1)); % min transient of the DE_PS
            mx       = max(monSDF(:,1,1)); % max transient of the DE_PS
            for monCond = 1:length(mfn)
