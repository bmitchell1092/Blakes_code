function ShadedLinePlotbyDepth(ASAbyDepth,corticaldepth,tm,N,offset,fill_deflection)
...
% baselined_ASA  = bsxfun(@minus,ASAbyDepth, mean(ASAbyDepth(:,tm<0),2));
scalingfactor = max(max(abs(ASAbyDepth),[],2)); 
scaled_ASA    = ASAbyDepth .* 1/scalingfactor;
...
for depth = size(ASAbyDepth,1):-1:1
    ...
    %{
    if ~isempty(N)
        text(max(x),bl,num2str(N(depth)),'HorizontalAlignment','left','FontName', 'Arial','FontSize', 8); hold on;
    end
%}
end

%    add scale bar and label
 ystr = sprintf('contacts indexed down from surface');
